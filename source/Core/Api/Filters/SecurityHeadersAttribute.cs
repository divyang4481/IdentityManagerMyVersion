    ...
    class SecurityHeadersAttribute : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuted(HttpActionExecutedContext actionExecutedContext)
        {
            ...
            )
            {
                ...
                if (EnableCsp)
                {
                    actionExecutedContext.Response.Headers.Add("Content-Security-Policy", "default-src 'self'; style-src 'self' 'unsafe-inline'; img-src *; connect-src *; script-src 'unsafe-eval' * ");
                }
            }
        }
    }
